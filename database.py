...
# Note: Database URL should be correctly configured in your .env file
# Ensure to set the database URL in your .env file
# Ensure to set the database URL in your .env file
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://your_correct_username:your_correct_password@localhost:5432/mydatabase')
# Use in-memory SQLite for testing when DATABASE_URL is not set
if DATABASE_URL == 'postgresql://your_username:your_password@localhost:5432/mydatabase':
...
if not DATABASE_URL:
    raise ValueError('DATABASE_URL not set, please check your .env configuration')
    # Ensure the DATABASE_URL is set
    global DATABASE_URL
    if not DATABASE_URL:
        raise ValueError('DATABASE_URL must be set in the environment.')
    return create_engine(DATABASE_URL)  # Lazy connect to DB
    global DATABASE_URL

    if not DATABASE_URL:
        raise ValueError('DATABASE_URL must be set in the environment.')
    if not DATABASE_URL:
        raise ValueError('DATABASE_URL must be set in the environment.')  # Lazy connect to DB
    if not DATABASE_URL:
        raise ValueError('DATABASE_URL must be set in the environment.')
    if not DATABASE_URL:
    ...
    DATABASE_URL = os.getenv('DATABASE_URL')  # Ensure DATABASE_URL is set in the environment
        ...
        DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://your_username:your_password@localhost:5432/mydatabase')
        DATABASE_URL = os.getenv('DATABASE_URL', 'sqlite:///:memory:')  # Use in-memory SQLite for testing
...
# Update the database URL with correct credentials
DATABASE_URL = os.getenv('DATABASE_URL', 'sqlite:///:memory:')  # Using in-memory SQLite for testing
